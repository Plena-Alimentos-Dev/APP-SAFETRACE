...
const BtnDivDes = styled.div`
  ...
  input[type="file"] {
    display: none; /* Oculta o campo de entrada de arquivo */
  }
  ...
  /* Estilo para exibir o nome do arquivo selecionado */
  #nome-arquivo-des {
    margin-top: 10px;
    font-weight: bold;
...
const enviarArquivoParaAPI = async (file: File) => { // Alterei 'arquivo' para 'file' aqui
    try {
      ...
      const resposta = await axios.post('http://localhost:5432/desossa', formData, {
        headers: {
...
export function Desossa(){
    const handleFileInputChange = (e:React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files.length > 0) {
            const elemento = document.getElementById("nome-arquivo-des");
            if (elemento !== null) {
  ...
  const handleEnviarClick = () => {
    const inputArquivo: HTMLInputElement | null = document.getElementById("arquivo-des") as HTMLInputElement;
  ...
  return (
    <BtnDivDes>
      <h1>Selecione o arquivo de Desossa</h1>
      <label htmlFor="arquivo-des">
        Escolher arquivo
        <input
          type="file"
          id="arquivo-des"
